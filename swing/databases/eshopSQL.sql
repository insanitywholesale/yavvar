...
CREATE FUNCTION assign_weight_class() RETURNS void AS $$
    UPDATE Products
...
CREATE FUNCTION get_shipped_orders() RETURNS SETOF Orders AS $$
    SELECT * FROM Orders o WHERE o.OrderHasShipped = TRUE AND o.OrderHasArrived = FALSE;
$$ LANGUAGE SQL;

CREATE FUNCTION get_unshipped_orders() RETURNS SETOF Orders AS $$
...
CREATE FUNCTION add_product(varchar, float, integer, integer, varchar, float, float, boolean, boolean) RETURNS void AS $$
...
CREATE FUNCTION add_product_with_manufacturer(varchar, float, integer, varchar, float, float, boolean, boolean) RETURNS void AS $$

$$ LANGUAGE SQL;

--                                       (title    price  catID  desc     vers   weight sale     stock  )
CREATE FUNCTION add_product_with_category(varchar, float, integer, varchar, float, float, boolean, boolean) RETURNS void AS $$
    INSERT INTO Products ();
$$ LANGUAGE SQL;

--                          (name     desc   )
CREATE FUNCTION add_category(varchar, varchar) RETURNS void AS $$
    INSERT INTO Categories ();
$$ LANGUAGE SQL;

--                              (name     email    address  phone  )
CREATE FUNCTION add_manufacturer(varchar, varchar, varchar, varchar) RETURNS void AS $$
